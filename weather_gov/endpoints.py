...
class Base_Endpoint:
    def __init__(self, parent) -> None:
        ...
        self._zone_type = validations.VALID_ZONE_TYPES
...
class Gridpoints(Base_Endpoint):
    ...
    def forecast_hourly(self, wfo:str, x:int, y:int, features:list=None, **params):
        for feature in features:
            if feature not in self._features:
                raise ValueError(f"{feature} is not a valid feature")
        if wfo not in self._wfo:
    ...
    def stations(self, wfo:str, x:int, y:int, **params):
        if wfo not in self._wfo:
            raise ValueError(f"{wfo} is not a valid WFO")
...
class Stations:
    ...
    def id(self, stationID: str, **params):
        return self.parent.get(f"stations/{stationID}", params)
    
    def id_observations(self, stationID: str, **params):
        return self.parent.get(f"stations/{stationID}/observations", params)
    
    def id_observations_latest(self, stationID: str, **params):
        return self.parent.get(f"stations/{stationID}/observations/latest", params)
    
    def id_observations_time(self, stationID: str, time:str,  **params):
...
class Offices(Base_Endpoint):
    ...
    def id(self, officeID: str, **params):
    ...
    def headlines(self, officeID: str, **params):
        if officeID not in self._wfo:
            raise ValueError(f"{officeID} is not a valid WFO")
        return self.parent.get(f"offices/{officeID}/headlines", params)

    def headline_id(self, officeID: str, headlineID: str, **params):
        if officeID not in self._wfo:
            raise ValueError(f"{officeID} is not a valid WFO")
...
class Points:
    ...
    def id(self, pointID: str, **params):
...
class Radar(Base_Endpoint):
    ...
    def server_id(self, serverID: str, **params):
...
class Zones(Base_Endpoint):
    ...
    def type(self, zoneType: str, **params):
    ...
    def type_zone_id(self, zoneType: str, zoneID: str, **params):
        if zoneType not in self._zone_types:
            raise ValueError(f"{zoneType} is not a valid zone type")
        return self.parent.get(f"zones/types/{zoneType}/{zoneID}", params)
    
    def type_zone_id_forecast(self, zoneType: str, zoneID: str, **params):
        if zoneType not in self._zone_types:
            raise ValueError(f"{zoneType} is not a valid zone type")
        return self.parent.get(f"zones/types/{zoneType}/{zoneID}/forecast", params)

    def forecast_id_observations(self, zoneID: str, **params):
        return self.parent.get(f"zones/forecasts/{zoneID}/observations", params)
    
    def forecast_id_stations(self, zoneID: str, **params):
